...
#include "BTTaskMobFindSupportPatrol.h"
#include "../MonsterAIController.h"
#include "../Monster.h"
#include "NavigationSystem.h"
#include "BehaviorTree/BlackboardComponent.h"

UBTTaskMobFindSupportPatrol::UBTTaskMobFindSupportPatrol()
{
    NodeName = TEXT("SupportPatrol");
}

EBTNodeResult::Type UBTTaskMobFindSupportPatrol::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	auto Origin = Cast<AMonster>(OwnerComp.GetBlackboardComponent()->GetValueAsObject(AMonsterAIController::BattleMonsterKey));
	FNavLocation NextPatrol;
	if (Origin != nullptr) {

		// Orgin을 중심점으로 반지름이 600인 원 안에서 랜덤 좌표 생성
		if (NavSystem->GetRandomPointInNavigableRadius(Origin->GetActorLocation(), Origin->GetSupportPatrolRadius(), NextPatrol))
		{
