...
void UBTServiceBdAttackTrace::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (OwnerComp.GetBlackboardComponent()->GetValueAsBool(AMonsterAIController::IsAttackCoolKey) == false&&
		...
		OwnerComp.GetBlackboardComponent()->GetValueAsBool(ABdAIController::IsBattleRunKey) == false) {
...
bool UBTServiceBdAttackTrace::Attack1Trace(AMonster* Monster, UBehaviorTreeComponent& OwnerComp, FVector Center)
{
	...
	if (Monster->GetTestMode())
	{
		...
		TopLine.SetLocation(TopLine.GetLocation() + FVector(0.0f, 0.0f, Monster->GetAttack1Range().M_Atk_Height));
...
bool UBTServiceBdAttackTrace::StatueTrace(AMonster* Monster, UBehaviorTreeComponent& OwnerComp, FVector Center)
{
	if (Monster->GetTestMode())
	{
		FTransform BottomLine = Monster->GetTransform();
		...
		Monster->GetAIController()->DrawRadial(Monster->GetWorld(), BottomDebugMatrix, Monster->GetAttack2Range().M_Atk_Radius, 360.0f, FColor::Blue, 10, 0.1f, false, 0, 2);
		Monster->GetAIController()->DrawRadial(Monster->GetWorld(), TopDebugMatrix, Monster->GetAttack2Range().M_Atk_Radius, 360.0f, FColor::Blue, 10, 0.1f, false, 0, 2);
	}
