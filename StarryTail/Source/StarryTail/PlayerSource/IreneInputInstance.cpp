...
void UIreneInputInstance::AttributeKeywordReleased(const EAttributeKeyword Attribute)
{
	...
	{
		// 속성을 변화시키는 함수
		if (!Irene->IreneState->IsDodgeState() && !Irene->IreneState->IsAttackState() && !Irene->IreneState->IsSkillState() && !Irene->IreneState->IsChargeState() && !Irene->IreneState->IsDeathState() && Irene->IreneAttack->GetAttribute() != Attribute)
		{
